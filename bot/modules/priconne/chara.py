...
res = 'C:/Program Files/apache/public/res/'
...
try:
    gadget_equip = img('priconne/gadget/equip.png').open()
    gadget_star = img('priconne/gadget/star.png').open()
    gadget_star_dis = img('priconne/gadget/star_disabled.png').open()
    gadget_star_pink = img('priconne/gadget/star_pink.png').open()
    unknown_chara_icon = img(f'priconne/unit/icon_unit_{UNKNOWN}31.png').open()
...
def download_chara_icon(id_, star):
    ...
    save_path = os.path.join(res,f'priconne/unit/icon_unit_{id_}{star}1.png')
...
class Chara:
    ...
    def icon(self):
        ...
        res = img(f'priconne/unit/icon_unit_{self.id}{star}1.png')
        if not res.exist:
            res = img(f'priconne/unit/icon_unit_{self.id}31.png')
        if not res.exist:
            res = img(f'priconne/unit/icon_unit_{self.id}11.png')
        ...
        if not res.exist:
            res = img(f'priconne/unit/icon_unit_{self.id}11.png')
        if not res.exist:
            res = img(f'priconne/unit/icon_unit_{UNKNOWN}31.png')
        return res
