...
from ..priconne import chara, _pcr_data
...
DIR_PATH = os.path.join(os.path.expanduser(
    config.RES_PATH), 'priconne', 'unit')
DB_PATH = os.path.join(os.path.dirname(__file__),"poke_man_pcr.db")
...
def get_random_cards(origin_cards, row_num, col_num, amount, bonus, get_random_cards_func=get_random_cards_list, *args):
    ...
    for i in range(amount):
        ...
        base.save(os.path.join(res_path,'poke_man_pcr.png'))
    ...
    return card_counter, card_descs
...
async def poke_back(ev,message:Message):
    mid = message.id
    ...
    if not daily_limiter.check(guid) and poke_tip_cd_limiter.check(guid):
        ...
        await msg_send(mid,cid,f'<@{uid}> 你今天戳得已经够多的啦，再戳也不会有奇怪的东西掉下来的~')
        return
    ...
    card_counter, card_descs = get_random_cards(db.get_cards_num(gid, uid), row_num, col_num,
                                                        amount, True, get_random_cards_list, SUPER_RARE_PROBABILITY, RARE_PROBABILITY)
    dash = '----------------------------------------'
    msg = f'别戳了别戳了o(╥﹏╥)o <@{uid}> 这些卡送给你了, 让我安静会...\n{dash}\n获得了:\n'
    for i in card_descs:
        msg += (i + '\n')
    await msg_send(mid,cid,msg,res_url + '/poke_man_pcr.png')
...
async def auto_mix_card(ev,message:Message):
    # 参数识别
    s = ev.message.extract_plain_text()
    args = s.split(' ')
...
async def storage(ev, message:Message):
    mid = message.id
    cid = message.channel_id
    gid = message.guild_id
    uid = message.author.id
